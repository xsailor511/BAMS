...
<tr>
  ...
  <%
  	...
  	int mark = order1.getMark();
  	int id = order1.getId();
  %>
  <div id="input<jsp:getProperty name="order" property="id" />">
  <%
  if(mark==1){
	  
  
  %>
  已受理&nbsp;|&nbsp;<input type="button" class='mybutton' value="取消" onclick="unmarkOrder('<%=id%>')"/>
  <%
  }else{
  %>
  未受理&nbsp;|&nbsp;<input type="button" class='mybutton' value="受理" onclick="markOrder('<%=id%>')"/>
  <%
  }
  %>
  </div>
